...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { token, user } = await mockSignUp(email, password);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute -top-40 -right-40 w-[550px] h-[550px] bg-gradient-to-bl from-purple-400/25 to-purple-600/15 dark:from-purple-600/25 dark:to-purple-800/15 rounded-full blur-3xl animate-float"></div>

        {/* Large floating orb - bottom left */}
        <div className="absolute -bottom-40 -left-40 w-[650px] h-[650px] bg-gradient-to-tr from-blue-400/25 to-blue-600/15 dark:from-blue-600/25 dark:to-blue-800/15 rounded-full blur-3xl animate-float-delayed"></div>

        {/* Center accent orb */}
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-gradient-to-l from-purple-300/20 to-blue-300/20 dark:from-purple-700/20 dark:to-blue-700/20 rounded-full blur-3xl animate-float-slow"></div>

        {/* Small accent orbs */}
        <div className="absolute top-1/3 left-1/4 w-64 h-64 bg-purple-500/15 dark:bg-purple-400/15 rounded-full blur-2xl animate-pulse"></div>
        <div className="absolute bottom-1/4 right-1/3 w-72 h-72 bg-blue-500/15 dark:bg-blue-400/15 rounded-full blur-2xl animate-pulse" style={{ animationDelay: '1.5s' }}></div>

        {/* Rotating gradient ring */}
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] opacity-25">
          <div className="absolute inset-0 bg-gradient-to-l from-transparent via-purple-500/25 to-transparent dark:via-purple-400/25 rounded-full animate-rotate-slow"></div>
        </div>

        {/* Additional sparkle effects */}
        <div className="absolute top-24 right-24 w-32 h-32 bg-gradient-to-bl from-violet-400/20 to-purple-500/20 dark:from-violet-600/20 dark:to-purple-700/20 rounded-full blur-xl animate-pulse" style={{ animationDelay: '0.5s' }}></div>
        <div className="absolute bottom-28 left-28 w-40 h-40 bg-gradient-to-tr from-blue-400/20 to-cyan-500/20 dark:from-blue-600/20 dark:to-cyan-700/20 rounded-full blur-xl animate-pulse" style={{ animationDelay: '2.5s' }}></div>
      </div>
            ...
            {error && (
              <div className="rounded-xl bg-gradient-to-r from-red-50 to-red-100 dark:from-red-900/30 dark:to-red-800/20 p-5 border-2 border-red-300 dark:border-red-700/50 animate-shake shadow-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-red-900 dark:text-red-200 font-semibold">Sign Up Failed</p>
